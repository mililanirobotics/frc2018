...
//		}
	...
	void DriverTwo()
	{
		...
		if ((stick2.GetRawButton(7)))
		{
			...
			Lift2.Set(ControlMode::PercentOutput, 1); //value to stop at is -3400
			if (encoder.GetDistance() < -3400)
			{
				Lift1.Set(ControlMode::PercentOutput, 0);
				Lift2.Set(ControlMode::PercentOutput, 0);
			}

			if(encoder.GetDistance() <= -2500 && encoder.GetDistance() > -3000) //50% to 75%
			{
				Lift1.Set(ControlMode::PercentOutput, 0.4);
				Lift2.Set(ControlMode::PercentOutput, 0.4);
			}
			else if(encoder.GetDistance() < -2500) //Lower 50%
			{
		...
		else if (stick2.GetRawButton(8))
		{
			...
			{
				Lift1.Set(ControlMode::PercentOutput, -0.2);
				Lift2.Set(ControlMode::PercentOutput, -0.2);
			}
		...
		else
		{
			Lift1.Set(ControlMode::PercentOutput, 0);
			Lift2.Set(ControlMode::PercentOutput, 0);
			//std::cout<<encoder.GetDistance()<<std::endl;
		}
